...
pbarush: {
    ...
    logo: "https://static.wikia.nocookie.net/russel/images/0/00/PBA_Rush_Logo_2016.png/revision/latest/scale-to-width-down/250?cb=20250217140355https://static.wikia.nocookie.net/russel/images/0/00/PBA_Rush_Logo_2016.png/revision/latest/scale-to-width-down/250?cb=20250217140355",
    group:["entertainment"],
    },
...
let currentChannelKey = "kapamilya"; // Starting channel
const tabs = ["all", "news", "entertainment", "movies", "sports", "documentary", "cartoons & animations"];
...
function renderChannelButtons(filter = "") {
    ...
    sortedChannels.forEach(([key, channel]) => {
        ...
        const matchesGroup = selectedGroup === "all" || (Array.isArray(group) ? group.includes(selectedGroup) : group === selectedGroup);
        ...
        if (currentChannelKey === key) {
            ...
            nowPlayingSpan.textContent = "Playing";
            nowPlayingSpan.style.cssText = `
                color:#00FF00;
                font-weight:bold;
                white-space:nowrap;
                margin-left:10px;
                ...
                flex-shrink: 0;
            `;
            btn.appendChild(nowPlayingSpan);
        }
...
function loadChannel(key) {
    ...
    jwplayer("video").on("error", (e) => console.error("JWPlayer Error:", e.message));
}
...
function setupWelcomeModal() {
    // 1. Check if the modal has already been shown in this session. If so, do nothing.
    ...
    if (!modal || !closeBtn || !countdownSpan) {
    ...
    const closeModal = () => {
        modal.classList.remove('active'); // Use class to hide
        clearInterval(countdownInterval); // Stop the countdown
        // 2. Set the flag in sessionStorage so it doesn't show again.
    ...
    // Close modal if a nav link is clicked
    ...
    // 3. Show the modal by adding the .active class and start the countdown
...
function setupQrCodeModal() {
    ...
    if (!qrCodeImage || !qrModal || !zoomedQrImage || !closeQrModalBtn) {
    ...
    qrCodeImage.addEventListener('click', () => {
        zoomedQrImage.src = qrCodeImage.src; // Set the image source
    ...
    qrModal.addEventListener('click', (e) => {
        // Close if the outer overlay is clicked
        if (e.target === qrModal) {
            closeModal();
        }
    });
...
function setupBackToTopButton() {
    ...
    if (!backToTopButton || !aboutSection) {
        console.error('Back to top button or about section not found');
        return;
    }
